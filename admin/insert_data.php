...
if (isset($_POST['submit'])) {
    ...
    $status = 1;
    $insert_record = mysqli_query($conn, "INSERT INTO movies (`movie_name`,`director`,`release_date`,`category`,`language`,`you_tube_link`,`action`,`description`,`show`,`image`,`status`)VALUES('" . $movie_name . "','" . $director_name . "','" . $release_date . "','" . $category . "','" . $language . "','" . $tailer . "','" . $action . "','" . $description . "','" . $show . "','" . $filename . "','" . $status . "')");
    if (!$insert_record) {
        echo "unsuccesfull";
    } else {
...
if (isset($_POST['updatemovie'])) {
    ...
    $insert_record = mysqli_query($conn, "UPDATE `movies` SET `movie_name` = '$edit_movie_name', `director` = '$edit_director_name', `category` = '$edit_category', `language` = '$edit_language',`you_tube_link` = '$tailer',`action` = '$action',`description` = '$description', `show` = '$edit_show', `image` = '$image' WHERE `id` = '$e_id'");
